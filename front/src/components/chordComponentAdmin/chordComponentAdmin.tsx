...
export const ChordComponentAdmin = ({chord}: {chord: Chord}) => {
    ...
    useEffect(() => {
      ...
      async function deleteChord(id: number) {
        try {
            const response = await axios.post(`${source}/deleteChord`, { id });
    ...
    async function editChord() {
        ...
        try {
            await axios.post(`${source}/uploadChords`, imageFormData, {
                headers: {
        ...
        try {
            await axios.post(`${source}/uploadChordsAudio`, audioFormData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
        ...
        try {
            const response = await axios.post(`${source}/editChord`, { chordName, image, audio, chord_id });
            return response.data;
        } catch (error) {
    ...
    return (
        <div id='chord'>
            ...
            <div id="loadImageChord">
                ...
                <h3>{t('choose-image')}</h3>
                <input type='file' onChange={handleAudioChange} accept='audio/*'></input>
                <h3>{t('choose-audio')}</h3>
            </div>
            <button onClick={() => clickChordHandler()} id="playChordButton">{t('play-chord')}</button>
            <div id='chordButtons'>    
                {!isSure ?
                    <button onClick={() => setIsSure(true)} id="deleteChord">{t('delete-chord')}</button> :
                    <button onClick={() => deleteChord(chord.id)} id="deleteChord" style={{color: 'beige'}}>{t('sure')}</button>
                }
                {(chord.chord !== chordName || image !== chord?.image || audio !== chord.audio) &&
                    <button id="editChord" onClick={editChord}>{t('save-changes')}</button>
                }
            </div>
        </div>
    );
